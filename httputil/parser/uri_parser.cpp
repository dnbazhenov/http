...
namespace httputil::parser
{
	...
	struct logger_t
	{
		void log_type(httputil::uri::uri_type t) {}
		void log_state(int) {}
	...
	inline void uri_parser::set_state(state_t s)
	{
		logger.log_state(s);
		_state = s;
	}

	inline void uri_parser::set_type(httputil::uri::uri_type t)
	{
		logger.log_type(t);
		_type = t;
	}

	inline size_t uri_parser::set_error(size_t p)
	{
		set_state(S_error);
		return p;
	}

	inline size_t uri_parser::set_done(size_t p)
	{
		set_state(S_done);
		return p;
	}

	size_t uri_parser::parse(const char* data, size_t size)
	{
	...
	bool uri_parser::update_port(char ch)
	{
		if (!is_digit(ch))
			return false;
	...
	bool uri_parser::pct_check(char ch)
	{
	...
	uri_parser::parse_results uri_parser::results() const
	{
